...
<<<<<<< HEAD
    ...
    const spritePalettes = {
        'default': {
            ...
            wire_normal: "url('assets/fio consertado - padrão.png')",
            wire_fixed: "url('assets/fio_concertado_padrao.png')",
        ...
        'high-contrast': { // Mapeado para TEA / Autismo
            ...
            wire_normal: "url('assets/fio consertado tea  .png')",
            wire_fixed: "url('assets/fio concertado tea .png')", 
            wire_broken: "url('assets/fio faiscado autismo .png')",
            scientist_default: "assets/espressão do cientista - TEA.png"
        },
        'colorblind-safe': { // Mapeado para Daltonismo
            ...
            wire_normal: "url('assets/fio consertado - dalto.png')",
    ...
    const AccessibilityManager = {
        ...
        updateUISprites: function() {
            ...
            root.style.setProperty('--sprite-fio-normal', this.getSprite('wire_normal')); // <-- ADICIONE ESTA LINHA
    ...
    function initAudio() {
        ...
        sounds = {
            'background': new Audio('assets/musica_fundo.mp3'),
            'tapeFix': new Audio('assets/som-de-fita.mp3'),
            'sparks': new Audio('assets/faiscas-e-curtos.mp3'),
            'alarm': new Audio('assets/alarme-eletrico.mp3'),
            'conclusion': new Audio('assets/musica-conclusao.mp3'),
            'error': new Audio('assets/erro.mp3')
        };

        sounds.background.loop = true;
        sounds.background.volume = 0.5;
        sounds.sparks.loop = true;
        sounds.sparks.volume = 0.3;
    ...
    /* AJUSTE AQUI - Versão melhorada da função */
=======
    // --- DICIONÁRIO DE IDIOMAS (I18N) ---
    const fase3LangData = {
        'pt': {
...
            panel: "url('Assets/Painel elétrico interno - cor padrão.png')",
            tape: "url('Assets/Fita isolante - cor padrão .png')",
            // --- CORREÇÃO AQUI --- (Troquei 'concertado' por 'consertado')
            wire_fixed: "url('Assets/fio consertado - padrão.png')",
            // --- FIM DA CORREÇÃO ---
            wire_broken: "url('Assets/fio faiscado - cor padrão.png')", // Este já estava certo
            arara_default: "Assets/Arara - cor padrão.png"
        },
        'high-contrast': { // Mapeado para TEA / Autismo
            panel: "url('Assets/Painel elétrico interno - TEA.png')",
            tape: "url('Assets/Fita isolante - TEA.png')",
            // --- CORREÇÃO AQUI TAMBÉM ---
            wire_fixed: "url('Assets/fio consertado tea .png')", // Era 'concertado'
            wire_broken: "url('Assets/fio faiscado autismo .png')",
            arara_default: null
        },
        'colorblind-safe': { // Mapeado para Daltonismo
...
            tape: "url('Assets/Fita isolante - Daltonismo .png')",
            // --- CORREÇÃO AQUI TAMBÉM ---
            wire_fixed: "url('Assets/fio consertado- Daltonismo.png')", // Era 'concertado'
            wire_broken: "url('Assets/fio faiscado - Daltônico .png')",
            arara_default: "Assets/Arara - cor padrão - Daltonismo.png"
        }
    };
    // --- GERENCIADOR DE ACESSIBILIDADE ---
    const AccessibilityManager = {
...
            if (araraSprite) {
                araraSprite.src = this.getSprite('arara_default');
            }
            const root = document.documentElement;
            root.style.setProperty('--sprite-panel', this.getSprite('panel'));
            root.style.setProperty('--sprite-tape', this.getSprite('tape'));
...
        // --- CORREÇÃO DE BUG: FORÇAR SFX LIGADO ---
        // Isso garante que seus efeitos sonoros toquem,
        // mesmo se estiverem desligados no localStorage.
        settings.sfx = true; 
        // ------------------------------------------
...
        };
        
        if (araraSpeechBubble) {
            araraSpeechBubble.textContent = araraMessages.start;
        }
    }
...
        // --- CORREÇÃO DE BUG: REBALANCEAMENTO DE VOLUME ---
        sounds.background.loop = true;
        sounds.background.volume = 0.2; // (Mais baixo, conforme pedido)
        
        sounds.sparks.loop = true; 
        sounds.sparks.volume = 0.7; // (Mais alto)

        sounds.alarm.loop = true; 
        sounds.alarm.volume = 1.0; // (Mais alto)
        
        sounds.error.volume = 0.9; // (Mais alto)
        sounds.tapeFix.volume = 1.0; // (Volume máximo)
        sounds.conclusion.volume = 0.8; // (Mais alto)
        // ------------------------------------------------
...
    
    function playSound(name) {
        // A checagem 'settings.sfx' acontece aqui
        if (!audioInitialized || !sounds[name] || !settings.sfx) return;
        sounds[name].currentTime = 0;
        sounds[name].play().catch(e => console.warn(`Erro ao tocar ${name}:`, e));
    }
...
    function startBackgroundMusic() {
        // A checagem 'settings.music' acontece aqui
        if (!audioInitialized || !settings.music) return;
        sounds.background.currentTime = 0;
        sounds.background.play().catch(e => console.log("Interação necessária para música."));
    }
...
<<<<<<< HEAD
        
        // --- Configuração do Layout (em %) ---
        const wireWidth = 6;    // <-- MUDANÇA: de 10 para 6
        const gapX = 0;       // <-- MUDANÇA: de 5 para 2.5
        const spacingY = 33;   // (Este pode ficar igual)
        const startingTop = 18; // (Este pode ficar igual)

        // --- Cálculo de Posição (Será recalculado automaticamente) ---
        // (5 * 6) = 30%
        const totalWireWidth = cols * wireWidth;
        // (4 * 2.5) = 10%
        const totalGapWidth = (cols - 1) * gapX;
        // (30 + 10) = 40% (Nosso novo bloco estreito)
        const totalBlockWidth = totalWireWidth + totalGapWidth;
        // (100 - 40) / 2 = 30% (Nova posição inicial)
        const startingLeft = (100 - totalBlockWidth) / 2;
        
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                const wire = document.createElement('div');
                wire.classList.add('wire', 'normal'); // <-- MUDANÇA (era 'fixed')
                
                wire.style.top = `${startingTop + r * spacingY}%`;
                wire.style.left = `${startingLeft + c * (wireWidth + gapX)}%`;
                
=======
        const spacingX = 9.8;
        const spacingY = 33;
        const startingTop = 14;
...
<<<<<<< HEAD
    function breakRandomWire() {
        ...
        // CORREÇÃO: Procurar por fios que podem quebrar (tanto 'normal' quanto 'fixed')
        const breakableWires = wires.filter(w => !w.classList.contains('broken'));
        if (breakableWires.length === 0) return;

        const randomWire = breakableWires[Math.floor(Math.random() * breakableWires.length)];
        
        // Remove os status antigos e adiciona 'broken'
        randomWire.classList.remove('normal', 'fixed'); 
    ...
    function fixWire(wire) {
        ...
        wire.classList.remove('broken', 'drag-over');
        wire.classList.add('fixed'); // <-- MANTÉM (agora 'fixed' é o fio com fita)
    ...
    // === EVENT LISTENERS (TOQUE PARA DISPOSITIVOS MÓVEIS) ===
    let isDraggingWithTouch = false;
    let lastTouchedWire = null;
    const tapeContainer = document.getElementById('tape-container');

    tape.addEventListener('touchstart', (e) => {
        if (!gameActive) return;
        e.preventDefault(); // Previne scroll e outros comportamentos padrão
        isDraggingWithTouch = true;

        // Adiciona feedback visual e prepara para mover
        tape.style.transform = 'scale(1.1)';
        tape.style.position = 'absolute';
        tape.style.zIndex = '1000';
        document.body.appendChild(tape); // Move para o body para evitar problemas de clipping

        // Posiciona a fita sob o dedo
        const touch = e.touches[0];
        tape.style.left = `${touch.clientX - tape.offsetWidth / 2}px`;
        tape.style.top = `${touch.clientY - tape.offsetHeight / 2}px`;

    }, { passive: false });

    document.addEventListener('touchmove', (e) => {
        if (!isDraggingWithTouch || !gameActive) return;
        e.preventDefault();

        const touch = e.touches[0];
        // Move a fita com o dedo
        tape.style.left = `${touch.clientX - tape.offsetWidth / 2}px`;
        tape.style.top = `${touch.clientY - tape.offsetHeight / 2}px`;

        // Esconde a fita temporariamente para detectar o elemento abaixo
        tape.style.visibility = 'hidden';
        const elementUnder = document.elementFromPoint(touch.clientX, touch.clientY);
        tape.style.visibility = 'visible';

        // Remove o realce do fio anterior se o dedo se moveu para fora dele
        if (lastTouchedWire && lastTouchedWire !== elementUnder) {
            lastTouchedWire.classList.remove('drag-over');
            lastTouchedWire = null;
        }

        // Adiciona realce se o dedo estiver sobre um fio quebrado
        if (elementUnder && elementUnder.classList.contains('wire') && elementUnder.classList.contains('broken')) {
            elementUnder.classList.add('drag-over');
            lastTouchedWire = elementUnder;
        }
    }, { passive: false });
    ...
    document.addEventListener('touchend', (e) => {
        ...
        // Lógica de "soltar"
        if (lastTouchedWire) {
            fixWire(lastTouchedWire);
            lastTouchedWire.classList.remove('drag-over');
        } else {
            // Verifica se soltou dentro da área do jogo, mas não em um fio
            const touch = e.changedTouches[0];
            const elementUnder = document.elementFromPoint(touch.clientX, touch.clientY);
            if (elementUnder && elementUnder.closest('#game-container')) {
                 handlePanelDrop(e);
            }
        }
        ...
        // Reseta a posição e estilo da fita, devolvendo-a ao seu container original
        tape.style.position = '';
        tape.style.zIndex = '';
        tape.style.left = '';
        tape.style.top = '';
        tape.style.transform = '';
        tapeContainer.insertBefore(tape, tapeContainer.firstChild);
    });
...
        randomWire.classList.add('broken');
        
        // (Regra: faisca-e-curtos)
        // A função playSound checa 'settings.sfx'
        if (sounds.sparks.paused) {
            playSound('sparks');
        }
...
        clearInterval(wireBreakInterval); 
        
        speak(didWin ? araraMessages.end_win : araraMessages.end_time);
        
        gamePanel.removeEventListener('dragover', handlePanelDragOver);
        gamePanel.removeEventListener('drop', handlePanelDrop);

        // (Regra: musica-conclusao)
        stopAllSounds(); 
        playSound('conclusion');
        
        const texts = fase3LangData[settings.language] || fase3LangData['pt'];
